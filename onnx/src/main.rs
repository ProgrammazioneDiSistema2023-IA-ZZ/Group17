...
use read_proto::proto_structure::Proto;
use read_proto::proto_structure::Value;
...
fn main() {
    ...
    match create_struct_from_proto_file(file_path.to_string()) {
        Ok(result) => {
            read_onnx(&result);
        },
        Err(_) => {
            println!("ERRORE CREAZIONE STRUTTURA PROTO");
        }
    }
}
...
fn read_onnx(proto_structure: &Vec<Proto>) {
    ...
    let mut modelProto = ModelProto::new();
    let mut current_struct = "ModelProto".to_string();
    ...
    while counter < onnx_bytes.len() {
        ...
        if binary_string.len() < 8 {
            ...
            let wire_type = get_wire_type(last_three_digits);
